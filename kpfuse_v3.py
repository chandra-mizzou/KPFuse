...
class FusionNet(nn.Module):
    """
    KPFuse v3:
    - 3-layer encoder/decoder (reduced from v2's 4)
    - 3 outputs:
    ...
    def __init__(
        ...
        ch2 = base_ch * 2
        ch3 = bottleneck_ch
        # Keep v2 argument names:
        # - attn_depth_l3/sr_l3 => stage-2 (mid-level) fusion
        ...
        # Multi-scale fusion stages (L2 + L3/bottleneck)
    ...
    def forward(self, v, i, vis_kmap=None, ir_kmap=None, return_aux: bool = False, return_all: bool = False):
        ...
        i1 = self.i1(i)
        i2 = self.i2(i1)
        # Mid-level fusion (stage-2).
        v2_f = self._run_fusion_stage(v2, i2, self.fusion_blocks_l2, vis_kmap, ir_kmap)

        v3 = self.v3(v2_f)
        i3 = self.i3(i2)
        # Bottleneck fusion (stage-3).
        v3_f = self._run_fusion_stage(v3, i3, self.fusion_blocks_l3, vis_kmap, ir_kmap)

        u3 = self.up3(v3_f)
        if u3.shape[-2:] != v2_f.shape[-2:]:
            u3 = F.interpolate(u3, size=v2_f.shape[-2:], mode="bilinear", align_corners=False)
        d3 = self.c3(torch.cat([u3, v2_f, i2], dim=1))
...
def parse_args():
    p = argparse.ArgumentParser("KPFuse v3 trainer")
    ...
    p.add_argument("--ckpt", type=str, default="best_kpfuse_v3.pth")
    p.add_argument("--preview-dir", type=str, default="kpfuse_v3_previews")
    ...
    p.add_argument("--gate-alpha-vis", type=float, default=0.25)
    p.add_argument("--gate-alpha-ir", type=float, default=0.35)
    p.add_argument("--attn-bias-gamma", type=float, default=0.20)
    ...
    p.add_argument("--luma-pred-mix", type=float, default=0.06)
    p.add_argument("--pred-rgb-mix", type=float, default=0.04)
